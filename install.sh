...
# Defaults
APPNAME="ifconfig"
APPDIR="$HOME/.local/share/srv/docker/ifconfig"
DATADIR="$HOME/.local/share/srv/docker/ifconfig/files"
INSTDIR="$HOME/.local/share/dockermgr/ifconfig"
REPO="${DOCKERMGRREPO:-https://github.com/dockermgr}/ifconfig"
REPORAW="$REPO/raw/$REPO_BRANCH"
APPVERSION="$(__appversion "$REPORAW/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Setup plugins
HUB_URL="mpolden/echoip"
...
SERVER_PORT="${SERVER_PORT:-15406}"
SERVER_PORT_INT="${SERVER_PORT_INT:-8080}"
SERVER_PORT_ADMIN="${SERVER_PORT_ADMIN:-16000}"
SERVER_PORT_ADMIN_INT="${SERVER_PORT_ADMIN_INT:-8080}"
...
if [ -f "$INSTDIR/docker-compose.yml" ] && cmd_exists docker-compose; then
  ...
  __sudo docker run -d \
    ...
    "$HUB_URL" -H X-Forwarded-For -a /data/GeoLite2-ASN.mmdb -c /data/GeoLite2-City.mmdb -f /data/GeoLite2-Country.mmdb -t /config/web &>/dev/null
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Install nginx proxy
