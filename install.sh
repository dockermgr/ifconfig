...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202304051137-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.com
# @@License          :  WTFPL
# @@ReadME           :  install.sh --help
# @@Copyright        :  Copyright: (c) 2023 Jason Hempstead, Casjays Developments
# @@Created          :  Wednesday, Apr 05, 2023 11:37 EDT
# @@File             :  install.sh
# @@Description      :  Container installer script for ifconfig
...
REPO="${DOCKERMGRREPO:-https://github.com/dockermgr}/ifconfig"
APPVERSION="$(__appversion "$REPO/raw/${GIT_REPO_BRANCH:-main}/version.txt")"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Defaults variables
APPNAME="ifconfig"
export APPDIR="$HOME/.local/share/srv/docker/ifconfig"
export DATADIR="$HOME/.local/share/srv/docker/ifconfig/rootfs"
export INSTDIR="$HOME/.local/share/CasjaysDev/dockermgr/ifconfig"
...
__docker_gateway_ip() { sudo docker network inspect -f '{{json .IPAM.Config}}' ${HOST_DOCKER_NETWORK:-bridge} 2>/dev/null | jq -r '.[].Gateway' | grep -v '^$' | head -n1 | grep '^' || echo '172.17.0.1'; }
...
HUB_IMAGE_URL="casjaysdevdocker/ifconfig"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set container hostname and domain - Default: [ifconfig.$SET_HOST_FULL_HOST] [$SET_HOST_FULL_DOMAIN]
CONTAINER_HOSTNAME=""
...
if [ -z "$CONTAINER_NAME" ]; then
  CONTAINER_NAME="$(__container_name || echo "casjaysdevdocker/ifconfig-$HUB_IMAGE_TAG")"
fi
...
if [ -n "$IFCONFIG_USERNAME" ]; then
  CONTAINER_USER_NAME="$IFCONFIG_USERNAME"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
  CONTAINER_USER_NAME="${IFCONFIG_USERNAME:-${CONTAINER_USER_NAME:-$DEFAULT_USERNAME}}"
fi
if [ -n "$CONTAINER_USER_NAME" ]; then
...
if [ -n "$IFCONFIG_PASSWORD" ]; then
  CONTAINER_USER_PASS="$IFCONFIG_PASSWORD"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
  if [ "$CONTAINER_USER_PASS" = "random" ]; then
    CONTAINER_USER_PASS="$(__password "${CONTAINER_PASS_LENGTH:-16}")"
  fi
  CONTAINER_USER_PASS="${IFCONFIG_PASSWORD:-${CONTAINER_USER_PASS:-$DEFAULT_PASSWORD}}"
fi
if [ -n "$CONTAINER_USER_PASS" ]; then
